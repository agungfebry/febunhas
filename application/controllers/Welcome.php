...
class Welcome extends CI_Controller {
	...
	{		
		// 6 artikel terbaru
		$data['artikel']          = $this->db->query("SELECT * FROM artikel,pengguna,kategori WHERE artikel_status='publish' AND artikel_author=pengguna_id AND artikel_kategori=kategori_id ORDER BY artikel_tanggal DESC LIMIT 6")->result();

		// kategori
		$data['kategori']	      = $this->db->query("SELECT * FROM kategori")->result();

		// event
		$data['event']	  = $this->db->query("SELECT * FROM event ORDER BY event_tanggal DESC LIMIT 6")->result();

		$data['sambutan']         = $this->db->query("SELECT profile_judul, profile_content FROM profile WHERE profile_judul LIKE '%sambutan%'")->row();
