...
class Dashboard extends CI_Controller
{
	...
	public function pengaturan_update()
	{
		...
		if ($this->form_validation->run() != false) {
			...
			$maps = explode(" ", $this->input->post('maps'));
			$phone = $this->input->post('phone');
			$email = $this->input->post('email');

			$where = array();

			$old_maps = substr($this->input->post('maps'), 0, 6);
			if ($old_maps == 'https://') {
				$url_maps = $old_maps;
			} else {
				$rm_src = str_replace('src="', '', $maps[1]);
				$rm_char = str_replace('"', '', $rm_src);
				$url_maps = $rm_char;
			}
			...
			if (!empty($_FILES['banner']['name'])) {
				...
				if ($this->upload->do_upload('banner')) {
					// mengambil data tentang gambar banner yang diupload
					$gambar = $this->upload->data();

					$banner = $gambar['file_name'];

					$this->db->query("UPDATE pengaturan SET banner='$banner'");
				}
			}
			// logo
